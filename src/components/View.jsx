...
const View = ({videoUploadResponse,removeVideoResponseFromCategory,setRemoveVideoResponseFromView}) => {
  ...
  const categoryVideoDrop= async (e) =>{
    const {categoryId,video} = JSON.parse(e.dataTransfer.getData("dataShadre"))
    ...
    const updatedCategoryDetails = {id:categoryId,categoryName:data.categoryName,allVideos:updatedAllvideos}
    ...
    // videos must be inserted to allVideos
  ...
  return (
    <>
      <Row draggable={true} onDragOver={dragOverView} onDrop={e=>categoryVideoDrop(e)}>
        {
          allVideos.length>0 ? 
            allVideos?.map(video=>(
